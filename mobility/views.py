...
class UtenteViewSet(viewsets.ModelViewSet):
    ...
    def aggiorna_metodo_pagamento(self, request, pk=None):
        ...
        utente.save()
        return Response({'status': f'Metodo di pagamento salvato con successo: {nuovo_metodo}'}, status=status.HTTP_200_OK)
    ...
    def sospendi(self, request, pk=None):
        ...
        return Response({'status': f'Utente {utente.nome} {utente.cognome} sospeso. Motivo: {motivo}.'}, status=status.HTTP_200_OK)
    ...
    def riattiva(self, request, pk=None):
        ...
        utente.save()
        return Response({'status': f'Utente {utente.nome} {utente.cognome} riattivato con successo.'}, status=status.HTTP_200_OK)
...
>>>>>>> 9dc3e03df850fd3a51e04c79251c5135f55cc619
    ...
    def prenota(self, request, pk=None):
        ...
        if mezzo.stato != 'DISPONIBILE':
            return Response({'error': f'Impossibile prenotare il mezzo: stato attuale {mezzo.stato}.'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def blocca_remoto(self, request, pk=None):
        mezzo = self.get_object()
        nuovo_stato = request.data.get('stato', 'MANUTENZIONE')
        mezzo.stato = nuovo_stato
        mezzo.save()
        return Response({'status': f"Mezzo #{mezzo.id} impostato in {nuovo_stato}."}, status=status.HTTP_200_OK)
...
class PromozioneViewSet(viewsets.ModelViewSet):
    ...
    def valida(self, request):
        ...
        return Response({'valida': True, 'codice': promo.codice, 'tipo_sconto': promo.tipo_sconto, 'valore': str(promo.valore)}, status=status.HTTP_200_OK)
...
class SegnalazioneViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        return Response({"message": "Segnalazione ricevuta. Il mezzo è in manutenzione."}, status=status.HTTP_201_CREATED)
    ...
    def risolvi(self, request, pk=None):
        ...
        return Response({'status': f'Segnalazione #{segnalazione.id} risolta.', 'mezzo_rimesso_in_servizio': not altre_aperte}, status=status.HTTP_200_OK)
...
class CorsaViewSet(viewsets.ModelViewSet):
    ...
    def termina(self, request, pk=None):
        ...
        try:
            corsa.termina_corsa(lat_rilascio=lat, lng_rilascio=lng, promozione=promozione, guasto_certificato=guasto_certificato)
    ...
    def blocco_temporaneo(self, request, pk=None):
        corsa = self.get_object()
        if corsa.fine: return Response({'error': 'La corsa è già terminata.'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def costo_corrente(self, request, pk=None):
        ...
        return Response({'corsa_id': corsa.id, 'minuti_trascorsi': int(durata_minuti), 'costo_stimato_corrente': round(durata_minuti * tariffa_applicata, 2)}, status=status.HTTP_200_OK)
    ...
    def stima_preventiva(self, request):
        ...
        distanza_km = math.sqrt(dx**2 + dy**2) / 1000.0
        ...
        tempo_minuti = max((distanza_km / vel_media) * 60.0, 1.0)
        return Response({
    ...
    def aggiorna_posizione_gps(self, request, pk=None):
        corsa = self.get_object()
        if corsa.fine: return Response({'error': 'Corsa terminata.'}, status=status.HTTP_400_BAD_REQUEST)
        lat = request.data.get('latitudine')
        lng = request.data.get('longitudine')
        if lat is None or lng is None: return Response({'error': 'Coordinate mancanti.'}, status=status.HTTP_400_BAD_REQUEST)
        PosizioneGPS.objects.create(corsa=corsa, latitudine=float(lat), longitudine=float(lng))
        return Response({'status': 'Posizione registrata.'}, status=status.HTTP_201_CREATED)


def _zona_di(corsa):
    if corsa.area_rilascio_id: return corsa.area_rilascio.nome_zona
    mezzo = corsa.mezzo
    aree = list(Area_Urbana.objects.all())
    if not aree or mezzo.latitudine is None or mezzo.longitudine is None: return 'Sconosciuta'
    def dist(a):
        return math.sqrt(((mezzo.latitudine - a.latitudine) * 111000)**2 + ((mezzo.longitudine - a.longitudine) * 80000)**2)
...
class AnalyticsCO2View(APIView):
    def get(self, request):
        ...
        return Response({
            ...
            'co2_risparmiata_kg': round(km_green * CO2_AUTO_KG_PER_KM, 2),
...
class AnalyticsUtilizzoView(APIView):
    def get(self, request):
        ...
        per_tipo = dict(Corsa.objects.values_list('mezzo__tipo').annotate(n=Count('id')).order_by())
        per_ora = [0] * 24
        per_zona = {}
        for c in corse:
        ...
        return Response({'corse_per_tipo': per_tipo, 'corse_per_ora': per_ora, 'corse_per_zona': per_zona, 'totale_corse': corse.count()}, status=status.HTTP_200_OK)
...
class AnalyticsReportView(APIView):
    def get(self, request):
        ...
        durata_media_min = round(sum([(c.fine - c.inizio).total_seconds() / 60.0 for c in corse_concluse]) / corse_concluse.count(), 1) if corse_concluse.exists() else None

        return Response({
            'periodo_giorni': giorni,
            'ricavi_totali_eur': round(float(ricavi), 2),
            'numero_corse': corse_periodo.count(),
            'numero_segnalazioni': Segnalazione.objects.filter(data_segnalazione__gte=da).count(),
            'efficienza': {
                'corse_per_mezzo': round(corse_periodo.count() / tot_mezzi, 2) if tot_mezzi else 0,
...
class OperatoreAllerteView(APIView):
    def get(self, request):
        ...
        allerte = [{'tipo': 'BATTERIA_SCARICA', 'mezzo_id': m.id, 'messaggio': f"{m.tipo} #{m.id}: batteria al {m.batteria}%"} for m in batteria_scarica] + \
                  [{'tipo': 'GUASTO', 'mezzo_id': m.id, 'messaggio': f"{m.tipo} #{m.id}: in manutenzione"} for m in guasti]
        return Response({'totale': len(allerte), 'allerte': allerte}, status=status.HTTP_200_OK)

class OperatoreConsoleView(APIView):
    def get(self, request):
        return Response({
            'segnalazioni_aperte': SegnalazioneSerializer(Segnalazione.objects.filter(risolta=False).order_by('-data_segnalazione'), many=True).data,
            'chat_aperte_count': ChatTicket.objects.filter(risolto=False).count(),
            'allerte_attive_count': Mezzo.objects.filter(batteria__lt=10).exclude(stato='MANUTENZIONE').count() + Mezzo.objects.filter(stato='MANUTENZIONE').count(),
        }, status=status.HTTP_200_OK)


class RoutingView(APIView):
    def post(self, request):
        origine, destinazione, tipo_mezzo = request.data.get('origine'), request.data.get('destinazione'), request.data.get('tipo_mezzo', 'SCOOTER')
        if not (isinstance(origine, list) and isinstance(destinazione, list) and len(origine) == 2 and len(destinazione) == 2):
            return Response({'error': 'Fornire origine e destinazione come [lat, lng].'}, status=status.HTTP_400_BAD_REQUEST)
        aree_vietate = list(Area_Urbana.objects.filter(tipologia__in=['VIETATA', 'CANTIERE']))
        return Response(RoutingService().calcola_percorso([float(origine[0]), float(origine[1])], [float(destinazione[0]), float(destinazione[1])], tipo_mezzo=tipo_mezzo, aree_vietate=aree_vietate), status=status.HTTP_200_OK)
...
>>>>>>> 9dc3e03df850fd3a51e04c79251c5135f55cc619
    ...
    def get_queryset(self):
        ...
        if utente_id: qs = qs.filter(utente_id=utente_id)
        if self.request.query_params.get('aperti') == 'true': qs = qs.filter(risolto=False)
        return qs
    ...
    def create(self, request, *args, **kwargs):
        ...
        bot_text = self._risposta_bot(msg.messaggio)
        bot_msg = ChatTicket.objects.create(utente=msg.utente, messaggio=bot_text, da_bot=True)

        return Response({'messaggio_utente': self.get_serializer(msg).data, 'risposta_bot': self.get_serializer(bot_msg).data}, status=status.HTTP_201_CREATED)

    @action(detail=True, methods=['post'])
    def rispondi(self, request, pk=None):
        ticket = self.get_object()
        testo = request.data.get('messaggio')
        if not testo: return Response({'error': 'Messaggio mancante.'}, status=status.HTTP_400_BAD_REQUEST)

        operatore = User.objects.filter(pk=request.data.get('operatore')).first() or (request.user if request.user.is_authenticated else None) or User.objects.filter(is_staff=True).first() or User.objects.first()
...
class MeteoView(APIView):
    def get(self, request):
        condizione = random.choice(['SOLE', 'PIOGGIA', 'VENTO', 'NUVOLOSO'])
        consigli = {
            'SOLE': ('BICI', 'Giornata perfetta per pedalare all\'aria aperta!'),
            'PIOGGIA': ('AUTO', 'Piove! Riparati prendendo un\'auto della flotta.'),
            'VENTO': ('AUTO', 'Troppo vento per mezzi a due ruote. Usa l\'auto.'),
            'NUVOLOSO': ('SCOOTER', 'Non piove ancora, vai veloce in monopattino.')
        }
        return Response({
